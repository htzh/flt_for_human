...
fibre — 005 §2's dictionary in one line. Composed with $`Y`$, a loop permutes the roots
and leaves $`X`$ fixed, so "a loop permutes the roots" and "the indeterminate $`Y`$ is
untouched" are the same statement. A loop is not a label either: the coset
...
$`X(1)`$; these three local monodromies generate the monodromy.
