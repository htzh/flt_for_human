...
The pair $`(m,n)`$ is read modulo $`p`$: $`(m,n)`$ and $`(m+pm',\,n+pn')`$ give the
same element of $`E_\tau[p]`$, hence the same $`\mathcal{C}`$. So we may take
$`(m,n) \in \mathbb{F}_p^2`$ with $`(m,n) \ne (0,0)`$, and for every nonzero
$`\lambda \in \mathbb{F}_p`$ the pairs $`(m,n)`$ and $`\lambda(m,n)`$ generate the same
$`\mathcal{C}`$ — the scalar ambiguity behind the word "lines" below.
...
**The roots are distinct, and two coefficients show it.** "Different cosets give
different $`j`$" is proved, and in the prime case it needs only two coefficients. The
code's normal form is `TS K e u`, which is $`j(uq^e)`$; then
...
**The local monodromies are §5's substitutions.** Going once around the cusp multiplies
a $`p`$-th root of the base uniformizer by $`\zeta_p`$; in §5's normalization, where the
base field is $`\mathbb{Q}(j(q^p))`$, that is exactly the twist
...
roots are known before the polynomial is — they are §5's slot list — so the existence
