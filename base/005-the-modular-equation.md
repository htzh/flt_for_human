...
Only the class of $`(m,n)`$ in $`\mathbb{F}_p^2`$ matters: for any
$`m', n' \in \mathbb{Z}`$,

$$\frac{(m+pm') + (n+pn')\tau}{p} \\;=\\; \frac{m+n\tau}{p} + (m'+n'\tau),$$

and $`m'+n'\tau \in \Lambda_\tau`$, so the two numerators are the same *point* of
$`E_\tau`$ and generate the same subgroup. This is an identity of points of
$`\mathbb{C}/\Lambda_\tau`$; a single $`\mathbb{Z}`$ may not be split across a sum
($`\mathbb{Z}(a+b\tau) \ne \mathbb{Z}a + \mathbb{Z}b\tau`$). For instance
$`\Lambda_\tau + \mathbb{Z}(1+\tau)/p`$ is one line, whereas
$`\Lambda_\tau + \mathbb{Z}/p + \mathbb{Z}\tau/p = \frac1p\Lambda_\tau`$ is the whole
$`p^2`$-element $`E_\tau[p]`$. Hence we may take $`0 \le m,n \le p-1`$ with
...
Different generators of the same $`\mathcal{C}`$ give the same *lattice*, hence the
same curve. Write $`x = (m+n\tau)/p`$ and let $`kx`$ be another generator ($`k`$ a unit
modulo $`p`$); from $`ak + bp = 1`$ we get $`x = a\,kx + b\,px`$, and
$`px = m+n\tau \in \Lambda_\tau`$. So each of $`x`$ and $`kx`$ lies in the lattice
generated by the other together with $`\Lambda_\tau`$, and the two lattices
$`\Lambda_\tau + \mathbb{Z}x`$ and $`\Lambda_\tau + \mathbb{Z}kx`$ are equal.
