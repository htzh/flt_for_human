...
| `FLTForHuman/ModularForms/LevelTwoCuspVanishing.lean` | no weight-2 cusp forms on `Γ(1)` and `Γ₀(2)` (`base/003`, both wrappers) | `ModularForm.S2_Gamma0_2_eq_zero` and `ModularForm.S2_Gamma0_one_eq_zero`, verbatim from their `Theorems/` wrappers, proved as **corollaries of the Sturm bound**: for weight `2` the bound is `⌊2·index/12⌋ = ⌊index/6⌋`, which is `0` at `Γ(1)` (index 1) and `Γ₀(2)` (index 3), and a cusp form has vanishing constant term (`CuspFormClass.qExpansion_coeff_zero`), so `sturm_bound_levelOne`/`sturm_bound_Gamma0` applied to `CuspForm.toModularFormₗ f` (then injectivity of the inclusion) gives `f = 0`. The level-2 case needs only the index value `(Γ₀ 2).index = 3` from `Gamma0TwoIndex.lean`; the bridges `Γ(1) = 𝒮ℒ` and `cuspForm_eq_zero_of_subgroup_eq` are kept. This supersedes the earlier norm route (the norm is the content of the general Sturm bound); the norm-only declarations were dropped. FLT `S_ModularForm_S2_Gamma0_2_eq_zero` 77–92, 176–206 + `S_ModularForm_S2_Gamma0_one_eq_zero` |
...
smallest port so far: two modules under `FLTForHuman/ModularForms/`
(`Gamma0TwoIndex`, `LevelTwoCuspVanishing`), now **corollaries of the Sturm
bound**, and the first entry into the **modularity/level-lowering** branch rather
than the Hecke cone. The superseded norm route's cusp-form norm is parked in the
`Reserve` library ([`Reserve/ModularForms/CuspFormNorm.lean`](Reserve/ModularForms/CuspFormNorm.lean)).
Its consumer is [spec/LevelTwoCuspConsumer.lean](spec/LevelTwoCuspConsumer.lean); the
measured record is [logs/level2-cusp-port.md](logs/level2-cusp-port.md).
