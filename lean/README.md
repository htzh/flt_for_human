...
| `FLTForHuman/ModularCurve/Defs/HeckeOperator.lean` | the Hecke degeneracy maps and the correspondence (SET-M1 m1) | `heckeAlphaBar` (the inclusion underlying the `N ∣ Nℓ` degeneracy) and `heckeBetaBar` (`q ↦ q ^ ℓ`), their integrality predicates `HeckeAlphaBarIntegral`/`HeckeBetaBarIntegral`, the divisor correspondence `heckeDivBar`/`heckePic0Bar` and the transposes. The pin's two generic supply lemmas `laurentBaseChange_mono`/`qExpand_mem_laurentBaseChange` are themselves `Theorems/` wrapper targets and are ported **publicly once** although FLT repeats them privately (`'` in `HeckeOperator`, `''` in `DegeneracyTower`); the other two supply lemmas dedup to `Defs/Laurent.lean`'s `coeffMap_qExpand`/`coeffEmb_qExpand`. `heckeAlphaBar = towerInclBar` is `rfl`. FLT `Def_ModularCurve_HeckeOperator` (191) |
| `FLTForHuman/ModularCurve/Defs/DegeneracyTower.lean` | the degeneracy tower and `HeckeExchangeAt` (SET-M1 m1) | `towerInclBar` (`N ∣ M`), `towerSubstBar` (`= towerInclBar ∘ heckeBetaBar`), the six definitional composites (`heckeAlphaBar_eq_towerInclBar` is `rfl`; `heckeBetaBar_eq_towerSubstBar` is a `Subtype.ext`), the square `heckeSquareBar_commutes`, and `HeckeExchangeAt` — the predicate the effort targets. The pin's private `laurentBaseChange_mono''` is imported from `HeckeOperator` instead of repeated. FLT `Def_ModularCurve_DegeneracyTower` (138) |
...
| `FLTForHuman/ModularCurve/Defs/AtkinLehner.lean` | the Fricke involutions and the `bar` cusp (SET-M1 m2) | `IsFrickeAut`/`frickeInvolution` and the `Full` pair `IsFrickeAutFull`/`frickeInvolutionFull`, the rational cusps `cuspZero`/`cuspZeroFull`, `order_coeffEmb_jq` and `cuspInftyBar`. Restores the pin's `SMul (F ≃ₐ[K] F) (Place K F)` (anonymous, from the dropped AC `DivisorClassGroup` block) through `SemilinearAut.ofAlgAut`. v4.34: `dif_pos`/`dif_neg` → `dite_eq_left`/`dite_eq_right`. FLT `Def_ModularCurve_AtkinLehner` (107) |
...
| `FLTForHuman/ModularCurve/Degree/Roof.lean` | the roof generation and the diagonal degree (SET-M3 m9) | `heckeRoof_adjoin_range_union_eq_top` (the roof's two legs generate the top field; `jqN_congr`/`mem_range_towerInclBar_iff`/`laurentBaseChange_adjoin_pair` + m6's `isIntegral_jqNModC_mul`) and `finrankAlong_towerSubstBar_comp_heckeAlphaBar` (the diagonal degree is the product; `fieldRange_heckeBetaBar`/`finrankAlong_heckeBetaBar`/`finrankAlong_towerInclBar_of_eq`/`finrankAlong_towerSubstBar_roof`). The three generic `AlgebraicCurve.finrankAlong` helpers (`finrankAlong_comp`, `finrankAlong_id`, `finrankAlong_eq_relfinrank_fieldRange`) are proved `private` here — **promotion candidates** for `AlgebraicCurve/Defs/Correspondence.lean`. The pin's `maxHeartbeats 6400000`/`synthInstance.maxHeartbeats 3200000` bumps are **not** needed: the port builds under the project's global `4000000` in **15 s**. FLT `S_ModularCurve_heckeRoof_adjoin_range_union_eq_top` (418 content, 356 prelude) + `S_ModularCurve_finrankAlong_towerSubstBar_comp_heckeAlphaBar` (441 content, 356 prelude) |
