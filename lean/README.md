...
| `FLTForHuman/ModularForms/CuspFormNorm.lean` | the norm of a cusp form (`base/003` move 2) | `CuspForm.norm` (the product of the translates `f ∣[k] g_q⁻¹` over `ℋ ⧸ 𝒢`, as a cusp form of weight `k · Nat.card (ℋ ⧸ 𝒢)`), the bridge `CuspForm.coe_norm_eq_coe_modularFormNorm`, and `CuspForm.norm_eq_zero_iff` (the norm vanishes only on zero forms; it is homogeneous, not linear). Mathlib's `ModularForm.norm` produces only a *modular* form; this module supplies the missing `zero_at_cusps'` from `CuspForm.translate` and `of_isFiniteRelIndex_conj`. v4.34: `CuspForm` no longer extends `ModularForm` (the `__ := ModularForm.norm` inheritance still fills `toFun`/`slash_action_eq'`/`holo'`), `tendsto_finset_prod` is `tendsto_finsetProd`, and the `coe_zero` lemmas are `map_zero`. FLT `S_ModularForm_S2_Gamma0_2_eq_zero` 18–70 |
...
| `FLTForHuman/ModularForms/LevelTwoCuspVanishing.lean` | no weight-2 cusp forms on `Γ₀(2)` (`base/003`, both wrappers) | `ModularForm.S2_Gamma0_2_eq_zero` and `ModularForm.S2_Gamma0_one_eq_zero`, verbatim from their `Theorems/` wrappers: the `Γ(1) = 𝒮ℒ` bridge, `cuspForm_eq_zero_of_subgroup_eq` (one copy for both pin files), `S6_levelOne_eq_zero`/`'`, the private quotient-card `Nat.card (Γ(1) ⧸ Γ₀(2)) = 3` and the public `IsFiniteRelIndex` instance (so consumers can apply `CuspForm.norm`), then the norm/weight-`6` assembly. The level-one statement skips the norm, its weight being already `< 12`. FLT `S_ModularForm_S2_Gamma0_2_eq_zero` 77–92, 176–206 + `S_ModularForm_S2_Gamma0_one_eq_zero` |
...
smallest port so far: three modules under `FLTForHuman/ModularForms/`
(`CuspFormNorm`, `Gamma0TwoIndex`, `LevelTwoCuspVanishing`), and the first entry
into the **modularity/level-lowering** branch rather than the Hecke cone. Its
consumer is [spec/LevelTwoCuspConsumer.lean](spec/LevelTwoCuspConsumer.lean); the
