...
| `FLTForHuman/ModularForms/SturmBound.lean` | the **Sturm bound** and its inputs (`math/012`) | `ModularForm.sturm_bound_Gamma0` (verbatim: a `Γ₀(N)` form whose `q`-expansion vanishes below `k · (Γ₀ N).index / 12` is zero) and `ModularForm.sturm_bound_of_isArithmetic` (the general arithmetic level, `k · relIndex 𝒮ℒ / 12`), via `ModularForm.eq_zero_of_lt_order_qExpansion_of_isArithmetic` (the norm to level one + `qExpansion_prod` + the order comparison), `ModularForm.levelOne_eq_zero_of_lt_order_qExpansion` (a corollary of mathlib's `ModularForm.sturm_bound_levelOne` + the coefficient reindexing), `Subgroup.IsArithmetic.exists_nat_mem_strictPeriods_conj` (the normal core's index is a strict period of every conjugate) and `CongruenceSubgroup.one_mem_strictPeriods_Gamma0`. The `relIndex_map_mapGL_W2D` and norm-analyticity helpers stay `private`. The final section adds the weight-`2` **cusp-form vanishing corollaries** `ModularForm.S2_Gamma0_{2,1}_eq_zero` (the `Γ(1) = 𝒮ℒ` bridge, `cuspForm_eq_zero_of_subgroup_eq`, `CuspFormClass.qExpansion_coeff_zero` and `CuspForm.toModularFormₗ`; the level-2 index value `(Γ₀ 2).index = 3` comes from `Gamma0TwoIndex.lean`). This is the cheap mandatory sub-cone of the finiteness targets (see `studies/hecke-finiteness-coverage.md`). The module also carries the two **coefficient-form** bounds `CuspForm.eq_zero_of_qExpansion_coeff_eq_zero` and `CuspForm.Gamma0_eq_zero_of_qExpansion_coeff_eq_zero` — FLT's `section SturmBound` declarations, re-derived from `sturm_bound_of_isArithmetic` (`relIndex = Nat.card` is `rfl`, `PowerSeries.nat_le_order` gives the order hypothesis, `k < 0` is `ModularForm.isZero_of_neg_weight`) instead of `CuspForm.norm` + `normCofactor` — and their consumer `CuspForm.qCoeffTrunc` plus the finite-dimensionality corollary `CuspForm.finiteDimensional_cuspForm` (the wrapper, closure 1/indeg 4, an endgame node) and the file-local `scoped instance finiteDimensional_Gamma0` it is proved from (the graph's 38-consumer node of that name is the sibling `S_CuspForm_finiteDimensional_Gamma0.lean`). FLT the six `S_` files (29 + 26 + 56 + 82 + 31 + 18 raw) + the two cusp-vanishing wrappers + the two coefficient-form bounds (`S_CuspForm_finiteDimensional_cuspForm.lean` 330–346) + `qCoeffTrunc`/`finiteDimensional_cuspForm` (ibid. 355–403 and `Thm_CuspForm_finiteDimensional_cuspForm.lean`) |
