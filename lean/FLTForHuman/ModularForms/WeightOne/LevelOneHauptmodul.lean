/-
  The level-one Hauptmodul leaves and the two big closure-1 packages.

  SET-6 order 1 ported the four leaves whose statements are the pinned wrappers
  `Theorems/Thm_<name>.lean` verbatim and whose proofs come from the matching
  `P2M/Sol/S_<name>.lean`, adapted to mathlib `v4.34.0`:
  ...
  * `WLight.linearIndependent_complex_of_qExpansion_rational` — rational
  ...
  SET-7 order 1 appends the two big self-contained closure-1 packages:

  * `WLight.levelOne_hauptmodul_package` — the level-one Hauptmodul/`j`/
    `q`-expansion-principle bundle (`S_` 1,186 lines);
  * `WLight.weierstrassP_qExpansion_package` — the `℘` `q`-expansion bundle
    (`S_` 858 lines).

  The third big package, `weierstrassP_torsion_qExpansion_package`, lives in
  `WeightOne/WeierstrassPTorsion.lean` (SET-7 order 2), which imports this
  module because its `wpTorsion`/`wpTorsionSeries` glue consumes
  ...
  https://github.com/anthropics/fermats-last-theorem/blob/aa2d8b3/P2M/Sol/S_WLight_levelOne_hauptmodul_package.lean
  https://github.com/anthropics/fermats-last-theorem/blob/aa2d8b3/P2M/Sol/S_WLight_weierstrassP_qExpansion_package.lean

  v4.34 drifts hit here: `if_pos`/`if_neg` → `ite_eq_left`/`ite_eq_right`,
  `if_true`/`if_false` → `ite_true`/`ite_false`, `Set.mem_setOf_eq` →
  `Set.mem_ofPred_eq`, `ModularForm.coe_sub` → `FunLike.coe_sub`,
  `ModularForm.IsGLPos.coe_smul` → `FunLike.coe_smul`,
  `ModularForm.IsGLPos.smul_apply`/`ModularForm.sub_apply` → `smul_apply`/
  `sub_apply`, `slash_action_eqn` → `SlashInvariantForm.slash_action_eqn`, an
  explicit `_root_.bernoulli` (under `open Polynomial`), and explicit
  `Matrix.SpecialLinearGroup.mapGL`. The pin's local
  `set_option maxHeartbeats 1600000`/`3200000` bumps are **not** transcribed:
  the project's global cap is `4000000`, above both.
-/
...
open Complex Real UpperHalfPlane Function Filter Polynomial Asymptotics
open ModularForm SlashInvariantForm SlashInvariantFormClass ModularFormClass CuspForm EisensteinSeries OnePoint Matrix.SpecialLinearGroup
...
private theorem levelOne_ratSpan (k : ℤ) :
  ...
  have hcoe_sub : (⇑(f - c₀ • Ek) : ℍ → ℂ) = ⇑f - ⇑(c₀ • Ek) :=
    FunLike.coe_sub f (c₀ • Ek)
...
private def holMulDiscPow (m : ℕ) (h : ℍ → ℂ) (hol : MDifferentiable 𝓘(ℂ) 𝓘(ℂ) h)
  ...
  slash_action_eq' := fun _ hγ => by
    ...
    rw [SL_slash, show ((γ : GL (Fin 2) ℝ)) = Matrix.SpecialLinearGroup.mapGL ℝ γ from rfl] at h1
    exact h1
...
private theorem j_surjective : Function.Surjective j := by
  ...
  have hinv : ∀ γ : SL(2, ℤ), hfn ∣[(0 : ℤ)] γ = hfn := by
    ...
    rw [hhdef, show (0 : ℤ) = 12 - 12 by norm_num, div_slash_SL2, SL_slash, SL_slash,
      SlashInvariantForm.slash_action_eqn _ _ hγ, SlashInvariantForm.slash_action_eqn _ _ hγ]
...
/-! ## 6. `WLight.weierstrassP_qExpansion_package` (SET-7 order 1)

The `℘` `q`-expansion bundle, statement verbatim from
`Theorems/Thm_WLight_weierstrassP_qExpansion_package.lean`, proof transcribed
from `P2M/Sol/S_WLight_weierstrassP_qExpansion_package.lean` (858 lines).
Helpers are `private`. -/

namespace WLight
namespace WeierstrassPPkg

open Complex Real Function Filter Polynomial Asymptotics
open ModularForm PeriodPair
open scoped MatrixGroups Manifold Topology UpperHalfPlane
...
private theorem tsum_one_div_sub_int_sq (w : ℂ) :
    ∑' n : ℤ, 1 / (w - n) ^ 2 = ∑' n : ℤ, 1 / (w + n) ^ 2 := by
  rw [← (Equiv.neg ℤ).tsum_eq]
  refine tsum_congr fun n ↦ ?_
  simp [sub_eq_add_neg]

private theorem lipschitz_two_of_im_pos {w : ℂ} (hw : 0 < w.im) :
    ∑' n : ℤ, 1 / (w + n) ^ 2 =
...
private theorem lipschitz_two_of_im_neg {w : ℂ} (hw : w.im < 0) :
    ∑' n : ℤ, 1 / (w + n) ^ 2 =
      (2 * π * I) ^ 2 * ∑' m : ℕ, (m : ℂ) * cexp (-(2 * π * I * w)) ^ m := by
  rw [← tsum_one_div_add_int_sq_neg, lipschitz_two_of_im_pos (w := -w) (by simpa using hw)]
  simp [mul_neg]

private theorem summable_nat_mul_cexp_pow {w : ℂ} (hw : 0 < w.im) :
    Summable fun m : ℕ ↦ (m : ℂ) * cexp (2 * π * I * w) ^ m := by
  simpa using summable_pow_mul_cexp 1 1 ⟨w, hw⟩

private theorem cexp_two_pi_I_mul_ne_one {x : ℂ} (hx : x ∈ Complex.integerComplement) :
    cexp (2 * π * I * x) ≠ 1 := by
  intro h
...
private theorem lipschitz_two_closed_of_im_pos {x : ℂ} (hx : 0 < x.im) :
    ∑' n : ℤ, 1 / (x + n) ^ 2 =
...
private theorem lipschitz_two_closed_of_im_neg {x : ℂ} (hx : x.im < 0) :
    ∑' n : ℤ, 1 / (x + n) ^ 2 =
      (2 * π * I) ^ 2 * (cexp (2 * π * I * x) / (1 - cexp (2 * π * I * x)) ^ 2) := by
...
private theorem lipschitz_two_closed {x : ℂ} (hx : x ∈ Complex.integerComplement) :
    ∑' n : ℤ, 1 / (x + n) ^ 2 =
      (2 * π * I) ^ 2 * (cexp (2 * π * I * x) / (1 - cexp (2 * π * I * x)) ^ 2) := by
...
private theorem summable_one_div_sub_int_sq (w : ℂ) :
    Summable fun n : ℤ ↦ 1 / (w - n) ^ 2 := by
  refine (Equiv.neg ℤ).summable_iff.mp ?_
  simpa [Function.comp_def, sub_eq_add_neg] using summable_one_div_add_int_sq w

private theorem tsum_int_one_div_sq : ∑' n : ℤ, 1 / ((n : ℂ)) ^ 2 = (π : ℂ) ^ 2 / 3 := by
  have h := two_mul_riemannZeta_eq_tsum_int_inv_pow_of_even (k := 2) le_rfl even_two
  simp only [Nat.cast_ofNat, riemannZeta_two] at h
  simp only [one_div, ← h]
  ring

private theorem two_pi_I_sq : (2 * π * I : ℂ) ^ 2 = -(4 * π ^ 2) := by
  rw [mul_pow, I_sq]; ring

private theorem neg_pi_sq_div_three_div : -((π : ℂ) ^ 2 / 3) = (2 * π * I) ^ 2 * (1 / 12) := by
  rw [two_pi_I_sq]; ring

private theorem tsum_row_split (z l₀ : ℂ) :
...
private theorem tsum_row_zero (z : ℂ) :
    ∑' d : ℤ, (1 / (z - d) ^ 2 - 1 / ((d : ℂ)) ^ 2) =
...
private theorem cexp_two_pi_I_mul_add (a b : ℂ) :
    cexp (2 * π * I * (a + b)) = cexp (2 * π * I * a) * cexp (2 * π * I * b) := by
  rw [mul_add, Complex.exp_add]

private theorem cexp_two_pi_I_mul_nat_mul (c : ℕ) (τ : ℂ) :
    cexp (2 * π * I * (c * τ)) = cexp (2 * π * I * τ) ^ c := by
  rw [← Complex.exp_nat_mul]
  ring_nf

private theorem cexp_two_pi_I_mul_neg (z : ℂ) :
    cexp (2 * π * I * (-z)) = (cexp (2 * π * I * z))⁻¹ := by
  rw [mul_neg, Complex.exp_neg]

private theorem lipschitz_row_pair {z τ : ℂ} (hτ : 0 < τ.im) (hz₁ : -τ.im < z.im) (hz₂ : z.im < τ.im)
    {c : ℕ} (hc : 0 < c) :
...
private theorem lipschitz_row_zero {z : ℂ} (hz : z ∈ Complex.integerComplement) :
...
private theorem lipschitz_rows_qExpansion {z τ : ℂ} (hτ : 0 < τ.im) (hz₁ : -τ.im < z.im)
...
private theorem hasSum_weierstrassP_prod (L : PeriodPair) (z : ℂ) :
...
private theorem summable_weierstrassP_prod (L : PeriodPair) (z : ℂ) :
    Summable (wpSummand L z) :=
  (hasSum_weierstrassP_prod L z).summable

private theorem weierstrassP_eq_tsum_tsum (L : PeriodPair) (z : ℂ) :
    weierstrassP L z =
      ∑' c : ℤ, ∑' d : ℤ,
...
private theorem summable_weierstrassP_row (L : PeriodPair) (z : ℂ) :
    Summable fun c : ℤ ↦
      ∑' d : ℤ, (1 / (z - (c * L.ω₁ + d * L.ω₂)) ^ 2 - 1 / (c * L.ω₁ + d * L.ω₂) ^ 2) :=
  (summable_weierstrassP_prod L z).prod

private theorem wpRow_periodPairOfTau_split (τ : ℍ) (z : ℂ) (c : ℤ) :
...
private theorem weierstrassP_periodPairOfTau_eq_row_sum (τ : ℍ) (z : ℂ) :
...
private theorem weierstrassP_periodPairOfTau_qExpansion (τ : ℍ) {z : ℂ}
...
private lemma torsionPt_hyps (τ : ℍ) {N a₁ a₂ : ℕ} (ha₁ : a₁ < N) (ha₂ : a₂ < N)
    (h0 : a₁ ≠ 0 ∨ a₂ ≠ 0) :
...
private theorem weierstrassP_torsionPt_qExpansion (τ : ℍ) {N a₁ a₂ : ℕ} (ha₁ : a₁ < N) (ha₂ : a₂ < N)
    (h0 : a₁ ≠ 0 ∨ a₂ ≠ 0) :
...
private lemma summable_wpTail {N a₁ : ℕ} (ha₁ : a₁ < N) (a₂ : ℕ) {τ : ℂ} (hτ : 0 < τ.im) :
...
private theorem weierstrassP_torsionPt_eq_tsum_prod (τ : ℍ) {N a₁ a₂ : ℕ} (ha₁ : a₁ < N) (ha₂ : a₂ < N)
    (h0 : a₁ ≠ 0 ∨ a₂ ≠ 0) :
    PeriodPair.weierstrassP (periodPairOfTau τ) (((a₁ : ℂ) * τ + a₂) / N) =
      (2 * π * I) ^ 2 *
        (zetaN N ^ a₂ * qN N τ ^ a₁ / (1 - zetaN N ^ a₂ * qN N τ ^ a₁) ^ 2 + 1 / 12 +
...
private lemma one_sub_zetaN_pow_mul_qN_pow_ne_zero {N a₁ a₂ : ℕ} (ha₁ : a₁ < N) (ha₂ : a₂ < N)
...
private lemma differentiable_qN (N : ℕ) : Differentiable ℂ (qN N) := by
  unfold qN; fun_prop

private lemma differentiable_wpTail (N a₁ a₂ : ℕ) (p : ℕ+ × ℕ+) :
...
private theorem differentiableOn_wpTorsionSeries {N a₁ a₂ : ℕ} (ha₁ : a₁ < N) (ha₂ : a₂ < N)
    (h0 : a₁ ≠ 0 ∨ a₂ ≠ 0) :
...
private lemma PeriodPair.ext_omega {L L' : PeriodPair} (h1 : L.ω₁ = L'.ω₁) (h2 : L.ω₂ = L'.ω₂) :
    L = L' := by
  cases L; cases L'; simp_all

end R1Bridge

theorem _root_.WLight.weierstrassP_qExpansion_package :
