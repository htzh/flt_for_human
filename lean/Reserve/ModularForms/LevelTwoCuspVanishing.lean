/-
  **Reserve — the norm route.** The endgame: **there are no weight-2 cusp forms
  on `Γ₀(2)`** (and the level-one analogue), assembled from the norm and the
  index.

  This is the pre-Sturm proof, restored verbatim from commit `4c4f588`
  (`lean/FLTForHuman/ModularForms/LevelTwoCuspVanishing.lean`), where it consumed
  `CuspFormNorm.lean` and `Gamma0TwoIndex.lean`. The main tree now proves the two
  theorems as corollaries of the Sturm bound
  (`FLTForHuman/ModularForms/SturmBound.lean`), whose general proof internalises
  the norm; this module is kept as the alternative route and stays verified by the
  `Reserve` library build.

  Because both routes declare the same wrapper theorems
  (`ModularForm.S2_Gamma0_{2,1}_eq_zero`), this module and
  `FLTForHuman.ModularForms.SturmBound` must not be imported together.
...
-/
import Reserve.ModularForms.CuspFormNorm
